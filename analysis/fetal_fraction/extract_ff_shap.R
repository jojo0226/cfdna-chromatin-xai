...
if (is_pca_lm) {
  ...
  # signed_vec keeps the DIRECTION of effect on FF (w_bin); imp_vec is |.|.
  #   w_bin > 0 : more coverage/openness -> higher predicted FF (fetal/placental-leaning)
  #   w_bin < 0 : more coverage/openness -> lower  predicted FF (maternal-leaning)
  signed_vec <- w
  imp_vec    <- abs(w)
  ...
  if (!is.na(x_path) && file.exists(x_path)) {
    ...
    mad_j  <- colMeans(abs(center))             # mean_i |x_ij - xbar_j|  (>= 0)
    signed_vec <- setNames(rep(0, length(w)), bin_name)
    imp_vec    <- setNames(rep(0, length(w)), bin_name)
    # signed exact linear-SHAP magnitude: sign is carried by w (mad_j >= 0)
    signed_vec[common] <- w[common] * mad_j
    imp_vec[common]    <- abs(w[common]) * mad_j
    imp_mode <- "pca_linear_shap_exact"
  }
  imp <- data.frame(feature = names(imp_vec),
                    mean_abs_shap = as.numeric(imp_vec),
                    signed_shap   = as.numeric(signed_vec),
  ...
  imp_mode <- "coef_only"; imp_vec <- abs(nz); signed_v <- nz; nm <- names(nz)
  if (!is.na(x_path) && file.exists(x_path)) {
    X <- as.matrix(read.csv(x_path, row.names = 1, check.names = FALSE))
    common <- intersect(names(nz), colnames(X))
    xbar <- colMeans(X[, common, drop = FALSE])
    mad_j <- colMeans(abs(sweep(X[, common, drop = FALSE], 2, xbar, "-")))
    imp_vec <- abs(nz[common]) * mad_j
    signed_v <- nz[common] * mad_j          # sign carried by coefficient (mad_j >= 0)
    nm <- common
    imp_mode <- "linear_shap_exact"
  }
  imp <- data.frame(feature = nm, mean_abs_shap = as.numeric(imp_vec),
                    signed_shap = as.numeric(signed_v),
                    stringsAsFactors = FALSE)
}
...
gmap <- imp[!is.na(imp$key), c("key", "mean_abs_shap", "signed_shap")]
# collapse duplicate keys: keep the row with the largest |shap| (carries its sign)
ord   <- order(gmap$key, -gmap$mean_abs_shap)
gmap  <- gmap[ord, ]
mapped <- gmap[!duplicated(gmap$key), c("key", "mean_abs_shap", "signed_shap")]
cat(sprintf("[map] %d/%d features -> genomic bins (%d dropped: non-genomic)\n",
            nrow(mapped), nrow(imp), nrow(imp) - nrow(mapped)))
cat(sprintf("[sign] %d bins w>0 (FF-up), %d bins w<0 (FF-down)\n",
            sum(mapped$signed_shap > 0), sum(mapped$signed_shap < 0)))

write.csv(mapped, out_path, row.names = FALSE)
cat(sprintf("[write] %s  (cols: key,mean_abs_shap,signed_shap -> run_ff_tissue_track.py)\n", out_path))
