...
def build_openness_atlas(tissue_panel: dict, source: str = "local",
                         ...
                         marks: tuple | list | None = None,
                         composite: str = "histone+dnase",
    ...
    marks:      which histone marks feed the H_<tissue> track. None -> all four
                ACTIVE_MARKS (H3K4me3, H3K4me1, H3K27ac, H3K36me3). Pass a subset to
                use e.g. only H3K4me3, or ("H3K4me3","H3K27ac") to combine two/three.
    composite:  what the per-tissue combined-openness track C_<tissue> is built from
                (see _add_contrasts): "dnase" (accessibility only), "histone"
                (selected marks only), or "histone+dnase" (default; z-mean of both).
                A tissue missing the requested modality falls back to whatever it has.

    Returns chrom,start,end,key + H_<tissue>/D_<tissue> + n_H_/n_D_ replicate counts +
    the reference *_specific contrasts + C_<tissue> per the `composite` choice.
    """
    if assembly is None:
        assembly = "hg38" if source == "local" else "hg19"
    if bin_size is None:
        bin_size = 500_000 if source == "local" else 50_000
    use_marks = tuple(ACTIVE_MARKS if marks is None else marks)
    ...
    for tissue, assays in tissue_panel.items():
        ...
        # active histone marks -> H_ (only the marks selected via `marks`)
        mark_tracks, n_marks = [], 0
        for mark in use_marks:
            if assays.get(mark):
    ...
    return _add_contrasts(out, composite=composite)


def _add_contrasts(out: pd.DataFrame, composite: str = "histone+dnase") -> pd.DataFrame:
    """Add the reference atlas composites/contrasts (contrast-first z, then average).

    composite controls the per-tissue C_<tissue> combined-openness track:
      "dnase"          -> accessibility only (ACC_ if present else D_)
      "histone"        -> selected histone marks only (H_)
      "histone+dnase"  -> z-mean of z(H_) and z(ACC_/D_)  (default)
    A tissue that lacks the requested modality falls back to whatever it has, so
    every tissue still gets a C_ column on the same z-scale.
    """
    def z(col):
    ...
    for t in base_tissues:
        ...
        has_h = f"H_{t}" in out
        # honor the requested composite; fall back to whatever the tissue has so a
        # tissue missing the requested modality still lands on the same z-scale.
        want_h = composite in ("histone", "histone+dnase") and has_h
        want_acc = composite in ("dnase", "histone+dnase") and acc_col is not None
        if want_h and want_acc:
            out[f"C_{t}"] = 0.5 * (z(f"H_{t}") + z(acc_col))
        elif want_h:
            out[f"C_{t}"] = z(f"H_{t}")
        elif want_acc:
            out[f"C_{t}"] = z(acc_col)
        elif has_h:                       # requested modality absent -> fall back
            out[f"C_{t}"] = z(f"H_{t}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--marks", default=None,
                    help="comma-separated histone marks to feed the H_ track, e.g. "
                         "'H3K4me3' or 'H3K4me3,H3K27ac'. Default: all four active "
                         f"marks ({','.join(ACTIVE_MARKS)}). Use with --composite to "
                         "pick histone-only or histone+DNase openness.")
    ap.add_argument("--composite", choices=["dnase", "histone", "histone+dnase"],
                    default="histone+dnase",
                    help="what the per-tissue C_ combined-openness track is built "
                         "from: dnase (accessibility only), histone (selected marks "
                         "only), or histone+dnase (default). A tissue missing the "
                         "requested modality falls back to whatever it has.")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()
    marks = tuple(m.strip() for m in args.marks.split(",")) if args.marks else None
    ...
    atlas = build_openness_atlas(PANEL, source=args.source, bin_size=args.bin_size,
                                 marks=marks, composite=args.composite)
