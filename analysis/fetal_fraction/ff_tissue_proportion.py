...
def tissue_proportion(imp: pd.DataFrame, atlas: pd.DataFrame,
    ...
    Ngrid = C.shape[0]
    _used_k = set()
    for topn in sorted(set(topns)):
        k = min(topn, n_avail)
        # a cutoff larger than the number of available bins collapses onto the
        # previous one (identical bins) -- skip it so the figure never draws two
        # identical bar groups, and tell the user why the cutoff is missing.
        if k in _used_k:
            print(f"[skip] top-{topn}: only {n_avail} bins available -> "
                  f"identical to top-{k}; omitted from results")
            continue
        _used_k.add(k)
        sel = ii["pos"].values[:k]
