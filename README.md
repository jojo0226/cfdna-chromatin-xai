...
### Maternal-vs-fetal origin marking — Wang MFI snATAC layer (`W_`, new in v0.6.3)

The tissue track above tells you *which* tissue the important bins are open in, but
a bulk openness atlas cannot say whether that chromatin is **fetal or maternal** — the
one distinction fetal-fraction analysis most needs. The `W_` layer supplies it: a
maternal-fetal interface (MFI) single-nucleus ATAC reference where **every cell type
carries an origin label**, from Wang et al. *Nature* 653:167–179 (2026),
doi:10.1038/s41586-026-10316-x — pseudobulk snATAC over **33 MFI cell types** (fetal
syncytio-/villous-/extravillous trophoblast + fetal stroma/endothelium/immune, and the
maternal decidua/immune compartment), 23 donors, 4.39 B fragments.

The layer ships as `reference/wang_mfi_openness_hg19_50kb.csv.gz` (57,633 bins × 33 `W_`
columns + `openness_fetal` / `openness_maternal` composites) with
`reference/manifest_wang_mfi.json`. It sits alongside the bulk DNase openness atlas
(`C_`), the Wang-2024 placental snATAC layer (`A_`), and the H3K27me3 repressive layer —
same 50 kb hg19 grid, same per-bin z-scoring, so the existing PC↔tissue machinery runs
against it with `--prefix W_`. The origin composites are built from **row-centered
specificity** rather than raw z (raw-z composites share an "open-everywhere" component
and correlate +0.91, useless for origin; specificity composites correlate −0.83 and form
a genuine fetal↔maternal axis).

```bash
# build the layer from the released tarball (aggregates 5 kb -> 50 kb by window midpoint)
python analysis/fetal_fraction/build_wang_mfi_atlas.py \
    --tar wang_mfi_atlas_hg19.tar.gz \
...
# per-PC + per-bin maternal/fetal origin calls with a bin-shuffle null and 2x2 Fisher
Rscript analysis/fetal_fraction/pc_origin_map.R --model ff_model.rds \
    --atlas    analysis/fetal_fraction/reference/wang_mfi_openness_hg19_50kb.csv.gz \
    --manifest analysis/fetal_fraction/reference/manifest_wang_mfi.json --outdir out_origin
```

`pc_origin_map.R` emits two things the tissue map cannot: a **per-PC origin score**
`[corr(L[,k],fetal) − corr(L[,k],maternal)]·sign(β_k)` (>0 = the PC's FF-up direction
opens fetal chromatin), and a **per-bin origin call + FF-direction × origin Fisher test**.
On the synthetic positive-control model (PC3 seeded placental β=+0.6, PC7 seeded monocyte
β=−0.5) it recovers the seeded axis exactly — PC3 → syncytiotrophoblast (fetal, z≈81),
PC7 → maternal monocytes (z≈38), and FF-up vs FF-down bins split fetal/maternal at
**OR = 17.5, p < 1e-3** — an independent confirmation of the FF sign convention (FF-up =
fetal/placental, FF-down = maternal). See `analysis/fetal_fraction/WANG_MFI_LAYER.md` for
the full recipe, grid-join details, and results.
...
analysis/fetal_fraction/ tissue-of-origin track: build_openness_atlas.py,
                        extract_ff_shap.R, select_signed_shap.py,
                        run_ff_tissue_track.py, ff_tissue_proportion.py,
                        pc_tissue_map.R + plot_pc_tissue.py (PC<->tissue mechanism),
                        plot_ff_tissue_dumbbell.py, plot_ff_coverage_shap.py,
                        build_wang_mfi_atlas.py + pc_origin_map.R (W_ origin layer),
                        FF_METHOD_WALKTHROUGH.md, WANG_MFI_LAYER.md,
                        reference/ (shipped hg19 atlases: C_ DNase, A_ placenta ATAC,
                        W_ Wang MFI, H3K27me3 repressive)
